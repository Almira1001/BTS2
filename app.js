...
const ACCOUNTS = {
  ...
  "CMI@IKK": { password: "Cak1329", role: "vendor", vendor: "PT Cakraindo Mitra International" },
  "ATM@IKK": { password: "Arg9207", role: "vendor", vendor: "PT Argo Trans Mandiri" },
  "PNL@IKK": { password: "Pun7031", role: "vendor", vendor: "PT Puninar Logistics" },
  "ETI@IKK": { password: "Ela1645", role: "vendor", vendor: "PT Elang Transportasi Indonesia" },
  "BRJ@IKK": { password: "Bim7086", role: "vendor", vendor: "PT Bimaruna Jaya" },
  "BSA@IKK": { password: "BSA2065", role: "vendor", vendor: "PT BSA Logistics Indonesia" },
  "TKJ@IKK": { password: "Tan1450", role: "vendor", vendor: "PT Tangguh Karimata Jaya" },
  "IPM@IKK": { password: "Int1842", role: "vendor", vendor: "PT Inti Persada Mandiri" },
  "GBU@IKK": { password: "Glo5279", role: "vendor", vendor: "PT Glory Bahana Universal" },
  "PSS@IKK": { password: "Put9028", role: "vendor", vendor: "PT Putra Sejahtera Sentosa" },
  "TRIS@IKK": { password: "Tri5267", role: "vendor", vendor: "PT Trisindo" },
  "LMN@IKK": { password: "Lin4914", role: "vendor", vendor: "PT Lintas Marindo Nusantara" },
};
// REVISI: Mengubah urutan Status Trucking
const STATUS_TRUCKING = [
  "Pending","Confirm Order","Reject","Muat Depo","muat gudang","Revo","gate in port"
];
...
function buildStatusDashboardInner(filterDate = null){
  const counts = {};
  const displayStatuses = ["Pending", "Confirm Order", "Reject", "Muat Depo", "muat gudang", "Revo", "gate in port"];
            ...
            <div class="card count-card">
              ...
              <div class="row">
                ${['today', 'monthly', 'overall'].map(period => `
                  <div class="col">
                    <div class="stat-card count-clickable" data-count-type="dn" data-count-period="${period}" style="border-color: #fee2e2; padding: 15px 10px; cursor:pointer;">
          ...
          <div class="col" style="grid-column: span 6;">
            <div class="card count-card">
              ...
              <div class="row">
                ${['today', 'monthly', 'overall'].map(period => `
                  <div class="col">
                    <div class="stat-card count-clickable" data-count-type="cont" data-count-period="${period}" style="border-color: #fee2e2; padding: 15px 10px; cursor:pointer;">
  ...
  try{
    ...
    if(headerEl && !document.getElementById('dashboard-status-rows')){
      ...
      const todayYesterdayDash = `
        <div class="card">
          <h3 style="margin:0 0 16px 0">📅 Daily Status</h3>
          
          <!-- TODAY Row dengan animasi kedap-kedip MERAH -->
          <div style="margin-bottom:20px;" class="blink-red-animation">
            <h4 style="margin:0 0 8px 0; color:#dc2626; font-weight:700;">🔴 TODAY - ${formatDisplayDate(todayISO)}</h4>
            ${buildStatusDashboardInner(todayISO)}
          </div>
          
          <!-- YESTERDAY Row -->
          <div>
            <h4 style="margin:0 0 8px 0; color:#6b7280; font-weight:600;">YESTERDAY - ${formatDisplayDate(yesterdayISO)}</h4>
            ${buildStatusDashboardInner(yesterdayISO)}
          </div>
        </div>
      `;
...
function restoreAvailability(orderId) {
  const order = state.orders.find(o => o.order_id === orderId);
  if (!order || !order.availability_reserved) return;
  
  const containers = state.containers[orderId] || [];
  const rejectedContainers = containers.filter(c => c.accept === false);
  
  if (rejectedContainers.length === 0) return;
  
  // Hitung jumlah container yang di-reject per size
  let rejected20 = 0, rejected40 = 0, rejectedCombo = 0;
  
  rejectedContainers.forEach(c => {
    if (!c.availability_restored) {  // Cek flag untuk hindari double restore
      if (c.size === "20ft") rejected20++;
      else if (c.size === "40ft/HC") rejected40++;
      else if (c.size === "Combo") rejectedCombo++;
      
      c.availability_restored = true;  // Mark sudah di-restore
    }
  });
  
  // Restore ketersediaan
  const dateISO = order.tgl_stuffing;
  if (dateISO && state.availability[dateISO]) {
    ...
    if (vendorAvail) {
      ...
      console.log(`✅ Restored availability: 20ft +${rejected20}, 40ft +${rejected40}, Combo +${rejectedCombo}`);
    }
  }
}
...
function restoreAvailabilityOnReject(orderId) {
  ...
  if (dateISO && state.availability[dateISO]) {
    const vendorAvail = state.availability[dateISO][order.vendor];
    if (vendorAvail) {
      // ✅ PENTING: Gunakan += untuk incremental, BUKAN = untuk overwrite
      vendorAvail["20ft"] = Number(vendorAvail["20ft"] || 0) + rejected20;
      vendorAvail["40ft/HC"] = Number(vendorAvail["40ft/HC"] || 0) + rejected40;
      vendorAvail["Combo"] = Number(vendorAvail["Combo"] || 0) + rejectedCombo;
      ...
      console.warn(`⚠️ Vendor availability not found for ${order.vendor} on ${dateISO}`);
    }
  } else {
...
function renderVendorHome(){
  ...
  content.innerHTML = `
    ...
    <!-- 📊 BARU: Dashboard Status -->
<div class="card" style="margin-top: 16px;">
  <h3 style="margin:0 0 16px 0;">📊 Dashboard Status</h3>
  <p style="margin:0 0 16px 0; font-size:0.85rem; color:#6b7280;">Periode: ${formatPeriodDisplay(state.vendorFilterStartDate || todayStr(), state.vendorFilterEndDate || todayStr())}</p>
      <div class="row" style="gap: 16px;">
        <div class="col" style="grid-column: span 4;">
          <div class="stat-card" style="background: linear-gradient(135deg, #dbeafe, #bfdbfe); border: 1px solid #60a5fa;">
            <div class="stat-num" style="color: #1e3a8a;">${totalAvailability}</div>
            <div class="stat-label" style="color: #1e40af;">Ketersediaan</div>
          </div>
        </div>
        <div class="col" style="grid-column: span 4;">
          <div class="stat-card" style="background: linear-gradient(135deg, #dcfce7, #bbf7d0); border: 1px solid #86efac;">
            <div class="stat-num" style="color: #065f46;">${totalAccept}</div>
            <div class="stat-label" style="color: #064e3b;">Accept Order</div>
          </div>
        </div>
        <div class="col" style="grid-column: span 4;">
          <div class="stat-card" style="background: linear-gradient(135deg, #fee2e2, #fecaca); border: 1px solid #fca5a5;">
            <div class="stat-num" style="color: #991b1b;">${totalReject}</div>
            <div class="stat-label" style="color: #7f1d1d;">Reject Order</div>
          </div>
        </div>
      </div>
    </div>
...
// Event listener untuk tombol Submit (Poin 2: Terlempar ke Add Detail)
    tbody.querySelectorAll('button[data-order-id-submit]').forEach(btn => {
        btn.onclick = () => {
            ...
            // ✅ REVISI: Ambil input REJECT (bukan accept) dari elemen UI
            tbody.querySelectorAll(`.input-reject[data-order-id="${oid}"]`).forEach(inp => {
                ...
                const accQty = total - rejQty; // Accept = Total - Reject
                ...
                targetConts.forEach((c, idx) => {
                    if (idx < accQty) {
                        ...
                        c.accept = false;
                        c.status = "Reject";
                        // ✅ PENTING: Jangan set availability_restored di sini
                        // Biarkan fungsi restoreAvailabilityOnReject yang handle
                    }
                });
            });

            // ✅ RESTORE AVAILABILITY untuk container yang di-reject
            ...
            render(); 
            toast("✓ Order berhasil disubmit. Ketersediaan telah diupdate.");
        };
    });
}
...
function renderOutstanding(){
  ...
  function drawMergedTable() {
    ...
    const fs = 'font-size:12px; padding:2px 4px; white-space:nowrap;';
    const thHtml = headerRow.map(h => `<th style="${fs}">${String(h == null ? '' : h)}</th>`).join('');

    const buildTbody = (rows) => rows.map(row => {
      const tds = headerRow.map((_, ci) => `<td style="${fs}">${String(row[ci] == null ? '' : row[ci])}</td>`).join('');
      return `<tr>${tds}</tr>`;
    }).join('');
    ...
    mergedDiv.innerHTML = `
      ...
      <div style="max-height:400px; overflow:auto; border:1px solid #e5e7eb; border-radius:6px;">
        <table style="width:100%; border-collapse:collapse;">
          <thead style="position:sticky; top:0; background:#f3f4f6; z-index:1;"><tr>${thHtml}</tr></thead>
          <tbody id="outMergedTbody">${buildTbody(allDataRows)}</tbody>
        </table>
      </div>
    `;

    document.getElementById('outSearchDN').addEventListener('input', (e) => {
      const kw = e.target.value.trim().toLowerCase();
      const filtered = kw
        ? allDataRows.filter(row => String(row[dnColIndex] == null ? '' : row[dnColIndex]).toLowerCase().includes(kw))
        : allDataRows;
...
function renderReconSummary() {
  ...
  function calcReconForDate(dateISO) {
    ...
    // Map: shippingPoint -> { plan20, plan40, realz20, realz40, planNW, realzNW }
    const spMap = {};

    ordersOnDate.forEach(order => {
      const sp = (order.shipping_point || 'Unknown').trim();
      if (!spMap[sp]) {
        spMap[sp] = { plan20: 0, plan40: 0, realz20: 0, realz40: 0, planNW: 0, realzNW: 0 };
      }

      // PLAN: ambil dari data outstanding berdasarkan DN order
      // Combo dihitung masuk ke 20 dikali 2
      const dns = order.no_dn || [];
      dns.forEach(dn => {
        const outData = getDataFromOutstanding(dn);
        spMap[sp].plan20 += Number(outData.partie20 || 0);
        spMap[sp].plan40 += Number(outData.partie40 || 0);
        spMap[sp].planNW += Number(outData.nw || 0);
      });

      // Jika outstanding tidak ada data, fallback ke jml yang di-order
      // Combo masuk ke 20 x2
      if (dns.length === 0 || (spMap[sp].plan20 === 0 && spMap[sp].plan40 === 0)) {
        spMap[sp].plan20 += Number(order.jml_20ft || 0) + (Number(order.jml_combo || 0) * 2);
        spMap[sp].plan40 += Number(order.jml_40ft || 0);
      }

      // REALZ: container yang accept === true, per size
      const containers = state.containers[order.order_id] || [];
      ...
      // Realz NW: estimasi dari NW outstanding * realz containers / plan containers
      // Atau bisa dihitung langsung dari accepted containers jika ada field nw
    ...
    content.innerHTML = `
        ...
        <div class="card">
            ...
            <div style="margin-top:16px; display:flex; justify-content:flex-end;">
              <button id="btnDownloadRevoReport" class="btn excel">📊 Excel</button>
            </div>
        </div>
    `;
      ...
      <div class="row" style="gap: 16px;">
        <div class="col" style="grid-column: span 6;">
          <div class="card" style="padding: 20px; background: linear-gradient(135deg, #dcfce7, #bbf7d0); border: 1px solid #86efac;">
            ...
            <p style="text-align: center; font-size: 1.2rem; font-weight: 700; color: #065f46; margin: 0;">+62 812-3456-7890</p>
        ...
        <div class="col" style="grid-column: span 6;">
          <div class="card" style="padding: 20px; background: linear-gradient(135deg, #dbeafe, #bfdbfe); border: 1px solid #60a5fa;">
            ...
            <p style="text-align: center; font-size: 1.2rem; font-weight: 700; color: #1e3a8a; margin: 0;">support@trucking.com</p>
            <button class="btn primary full" onclick="window.location.href='mailto:support@trucking.com'" style="margin-top: 12px;">📧 Kirim Email</button>
          </div>
        </div>
      </div>
